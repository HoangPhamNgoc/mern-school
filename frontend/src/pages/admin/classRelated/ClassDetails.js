...
const ClassDetails = () => {
    ...
    const ClassTeachersSection = () => <Typography>Teachers</Typography>;

    const ClassDetailsSection = () => (
        <>
            <Typography variant="h4" align="center" gutterBottom>Class Details</Typography>
            <Typography variant="h5" gutterBottom>This is Class {sclassDetails?.sclassName}</Typography>
            <Typography variant="h6" gutterBottom>Number of Subjects: {subjectsList.length}</Typography>
            <Typography variant="h6" gutterBottom>Number of Students: {sclassStudents.length}</Typography>
            {getresponse && <GreenButton variant="contained" onClick={() => navigate(`/Admin/class/addstudents/${classID}`)}>Add Students</GreenButton>}
            {response && <GreenButton variant="contained" onClick={() => navigate(`/Admin/addsubject/${classID}`)}>Add Subjects</GreenButton>}
        </>
    );
